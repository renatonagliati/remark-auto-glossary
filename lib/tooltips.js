...
const regexpTooltip = new RegExp('%%(.*)\\|(.*)%%');
...
export const generateTooltip = (glossaryFileContent, node, idx, parent) => {
  const match = node.value.match(regexpTooltip)
  ...
  // set the tooltip id
  const tooltipId = termId + '_' + Date.now();

  const tooltipTriggerEl = {
    type: 'html',
    value: `<a id="${tooltipId}" href="#" data-tooltip-html="${tooltipContent}" data-tooltip-place="top">${label}</a>`,
  }
